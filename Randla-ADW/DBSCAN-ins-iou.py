...
class PointCloudInstanceSegmenter:
    ...
    def perform_instance_segmentation(self):
        ...
        # 获取真实实例信息，这里只是简单示例，需根据实际真实标注逻辑修改
        self._get_ground_truth_instances()
    ...
    # 获取真实实例信息，这里只是简单模拟，实际需根据真实标注文件等解析
    def _get_ground_truth_instances(self):
        """
        模拟获取真实实例信息，实际应用中需根据真实标注数据（如专门的实例标注文件）来解析。
        这里简单根据语义标签，将每个语义类别视为一个“真实实例”（实际场景需更精细的真实实例标注逻辑）。
        """
        unique_labels = np.unique(self.labels)
        for label in unique_labels:
            if label == self.args.ignore_label:
            ...
            label_mask = self.labels == label
            label_points = self.points[label_mask]
            self.ground_truth_instances.append({
                'class_id': label,
                'points': label_points,
                'point_indices': np.where(label_mask)[0]
            })
    ...
    def _calculate_iou(self, pred_instance, gt_instance):
        """
        计算预测实例和真实实例的IoU。
        :param pred_instance: 预测出的实例字典，包含'points'等键
        :param gt_instance: 真实实例字典，包含'points'等键
        :return: IoU值
        """
        pred_points = pred_instance['points']
        gt_points = gt_instance['points']

        # 将点云转换为集合（基于坐标，这里简单处理为元组形式判断，也可用KDTree等更高效方式）
    ...
    def calculate_miou(self):
        """
        计算mIoU，遍历预测实例和真实实例，计算两两IoU并平均。
        :return: mIoU值
        """
        if not self.instances or not self.ground_truth_instances:
            print("警告：预测实例或真实实例为空，无法计算mIoU")
            return 0.0

        iou_sum = 0.0
        count = 0
        for pred_ins in self.instances:
            for gt_ins in self.ground_truth_instances:
                # 可根据类别ID先筛选，这里简单计算所有实例对
...
def main():
    ...
    try:
        ...
        # 计算并输出mIoU
        miou = segmenter.calculate_miou()*100
        print(f"计算得到的mIoU值为: {miou:.4f}")
