...
class ConfigS3DIS:
    ...
    sub_grid_size = 0.02  # preprocess_parameter
...
class ConfigSemantic3D:
    k_n = 16  # KNN  defualt knn
    num_layers = 5  # Number of layers
    num_points =5000  # Number of input points
    ...
    def get_adaptive_k(self, points):
        """根据点云密度计算自适应k值"""
        if len(points) == 0:
            return 8  # 默认最小值
        ...
        # 使用k_n作为最大k值
        min_k = 4  # 最小k值，固定
        max_k = self.k_n  # 从配置中获取最大k值
    
        norm_distance = min(avg_distance / 0.5, 1.0)
        adaptive_k = int(min_k + (max_k - min_k) * (1 - norm_distance))
        return adaptive_k
...
class DataProcessing:
    ...
    def get_class_weights(dataset_name):
        ...
        if dataset_name is 'S3DIS':
            ...
            num_per_class = np.array([122817, 420838, 599762, 1141018, 50926, 71888, 86103],
                                     dtype=np.int32)
